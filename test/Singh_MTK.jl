...
using Unitful
    ...
    @parameters t [unit = u"hr", description = "Independent variable (time in hours)"]
    Dt = Differential(t)
    ...
    R_tumor = (3 * TV_mm3/(4*pi))^(1/3)# [unit ="mm"]

    # @observed free_receptors = ((Ag_total) - C_ADC_b_ex_nM)
    ...
    Dt(X2_ADC_nmol) ~ (CLD_ADC/V1_ADC)*X1_ADC_nmol - (CLD_ADC/V2_ADC)*X2_ADC_nmol ,
    ...
    Dt(C_ADC_endolyso_cell_nM) ~ K_int_ADC*C_ADC_b_ex_nM - K_deg_ADC*C_ADC_endolyso_cell_nM,
    ...
    Dt(C_Drug_b_cell_nM) ~ K_on_Tub*C_Drug_f_cell_nM*(Tub_total - C_Drug_b_cell_nM) - K_off_Tub*C_Drug_b_cell_nM,
    ...
    Dt(C2_Drug_nM) ~ (CLD_Drug/V2_Drug)*C1_Drug_nM - (CLD_Drug/V2_Drug)*C2_Drug_nM,

    Dt(DAR) ~ -K_dec_ADC_plasma*DAR,
...
@named singh = ODESystem(eqs)

prob = ODEProblem(singh,[], (0.0, 100.0))
